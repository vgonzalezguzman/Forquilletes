...
const platToEdit = ref(props.platToEdit);
const title = ref(platToEdit.value.name);
const price = ref(platToEdit.value.price);
const rId = ref(platToEdit.value.rId);
...
const uploadPlat = async () => {
    try {
        ...
        formData.append('pId', platToEdit.value.id);
        formData.append('rId', rId.value);
        formData.append('title', title.value);
        formData.append('price', price.value);
        formData.append('type', selectedType.value);

        const response = await axios.post('/plats/edit', formData, {
            headers: {
...
const deletePlat = async () => {
    try {
        ...
        const response = await axios.post('/plats/delete', formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
                        ...
                        <div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <form class="w-full">
                                ...
                                <div class="flex w-full justify-end px-3">
                                    ...
                                    <button @click="deletePlat" type="button"
                                        class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium sm:mt-0 sm:w-auto sm:text-sm">
                                        Eliminar
                                    </button>
                                </div>
